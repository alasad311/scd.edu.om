...
export default function ProgramPa({majorBA,short,majorMA}) {
  ...
  const [programPlan, setProgramPlan] = useState<any>(null);
  const [courseDesc, setCourseDesc] = useState<any>(null);
  ...
  const handleClick = (id,string) =>
  {
      ...
      {
        case "BA":
          fetchData(id,"&major="+id,"major");
          break;
        case "MA":
          fetchData(id,"&major="+id,"major");
          break;
        case "SHORT":
          fetchData(id,"&short="+id,"short");
          break;
      }
  };
  const fetchData = async (id,program,type) => {

    if(type === "major")
    {
      const mrequest = await fetch('/api/program-major-details?id='+id+program);
      const majordetailsjsondata = await mrequest.json();
  
      const planyrequest = await fetch('/api/program-plan-year?id='+majordetailsjsondata[0]['md_id']);
      const majoryearcounter = await planyrequest.json();
  
      const plan = await fetch('/api/program-plan?id='+majordetailsjsondata[0]['md_id']);
      const majorplan = await plan.json();
  
      const courserequest = await fetch('/api/course-description?id='+majordetailsjsondata[0]['md_id']);
      const coursedec = await courserequest.json();
  
      const artrequest = await fetch('/api/program-artwork?id='+majordetailsjsondata[0]['md_id']);
      const artw = await artrequest.json();
      
      const hodreq = await fetch('/api/hod-dpt?id='+id);
      const hod = await hodreq.json();
  
      const staffreq = await fetch('/api/staff-dpt?id='+id);
      const staff = await staffreq.json();
      
      const reqirementreq = await fetch('/api/criteria?id='+majordetailsjsondata[0]['md_id']);
      const requirement = await reqirementreq.json();
  
      const feesreq = await fetch('/api/fees?id='+majordetailsjsondata[0]['md_id']);
      const fees = await feesreq.json();
  
      const finalfeereq = await fetch('/api/totalfees?id='+majordetailsjsondata[0]['md_id']);
      const finalfee = await finalfeereq.json();
  
      setProgramDetails(majordetailsjsondata);
      setProgramYerCounter(majoryearcounter);
      setProgramPlan(majorplan)
      setCourseDesc(coursedec)
      setArtwork(artw)
      setHod(hod)
      setStaff(staff)
      setRequirement(requirement)
      setFees(fees)
      setTotalFees(finalfee)
    }
    else
    {
      const mrequest = await fetch('/api/program-major-details?id='+id+program);
      ...
      const courserequest = await fetch('/api/course-short?id='+majordetailsjsondata[0]['md_id']);
      const coursedec = await courserequest.json();
        ...
        <div className="container-fluid">
          <div className="row" id="step-1-programs">
            <div className='col-lg-12'>
              <Image src="/assets/content/programs.jpg" width={2000} height={600} className="img-fluid" />
            ...
            <div className="row justify-center">
              {majorBA.map((e) => (
                <div className="col-12 col-md-6 col-lg-3 padding-0" key={e.id}>
                  <a href="#" className="program" onClick={() => handleClick(e.id,"MA") } data-major={e.id}   data-targert="">
                    <div className="course-card-style-3">
                      <div className="card">
                        <div className="thumb">
                        <Image className="card-img-top" src={"/assets/majors/"+e.major_image} width={250} height={250} alt={e.major_name} />
                        </div>
                    ...
                    <a href="#" className="program" onClick={() => handleClick(e.id,"MA") } data-major={e.id}   data-targert="">
                      <div className="course-card-style-3">
                        <div className="card">
                          <div className="thumb">
                          <Image className="card-img-top" src={"/assets/majors/"+e.major_image} width={250} height={250} alt={e.major_name} />
                            {/* <img className="card-img-top" src="https://via.placeholder.com/450x250" alt="Card image cap" /> */}
                          </div>
                          <div className="card-img-overlay">
                            <div className="card-body">
                            <h6>{e.major_name}</h6>
              ...
              <div className="row justify-center">
                {short.map((e) => (
                  <div className="col-12 col-md-6 col-lg-3 padding-0 " key={e.sc_id}>
                    <a href="#" className="program"  onClick={() => handleClick(e.sc_id,"SHORT") } data-shortcourse={e.sc_id} data-targert="">
                      <div className="course-card-style-3">
                        <div className="card">
                          <div className="thumb">
                          <Image className="card-img-top" src={"/assets/majors/"+e.sc_image} width={300} height={300} alt={e.sc_name} />
                            {/* <img className="card-img-top" src="https://via.placeholder.com/450x250" alt="Card image cap" /> */}
                          </div>
                          <div className="card-img-overlay">
                            <div className="card-body">
              ...
              <div className="col-lg-9">
                  ...
                  <div className={programDetails[0]['major_level'] === "MA" ? "tab-pane tab-pane-navigation displaynon" : "tab-pane tab-pane-navigation active"} id="generaloverview">
                    <div className= {programDetails[0]['id'] === 23 || programDetails[0]['id'] === 18 ? "row directionrtl" : "row"}>
                      <div className="col-lg-12">
                        <Image src={"/assets/content/overview.jpg"} width={1600} height={400} className="img-fluid padding-bottom-50" />
                  ...
                  <div className="row">
                    <div className="col-lg-4 fluid-col-lg-4 col-mb-4" >
                        <div className="fluid-col">
                            <Image src={"/assets/employees/"+hod[0]['staff_image']} width={400} height={400} className="img-fluid cardImg padding-bottom-20" />
                        </div>
                    </div>
                ...
                <div className="tab-pane tab-pane-navigation" id="requirements">
                  <div className="row padding-bottom-50">
                    <div className="col-lg-12">
                        <Image src={"/assets/content/requirement.jpg"} width={1600} height={400} className="img-fluid" />
                    </div>
                  </div>
                    {requirement &&
                      <div className="row">
